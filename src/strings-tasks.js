...
function findLongestWord(sentence) {
  return sentence.split(/\s+/).sort(function (a, b) {
...
function encodeToRot13(str) {
  ...
  const acResult = [];
  str.split('').forEach((i) => {
    let prom = i.charCodeAt();
    if ((prom >= 65 && prom <= 77) || (prom >= 97 && prom <= 109)) {
      prom = i.charCodeAt() + 13;
      result.push(prom);
    } else if ((prom >= 78 && prom <= 90) || (prom >= 109 && prom <= 122)) {
      prom = i.charCodeAt() - 13;
      result.push(prom);
    } else {
      result.push(prom);
  ...
  result.forEach((j) => {
    acResult.push(String.fromCharCode(j));
  });
  return acResult.join('');
}
